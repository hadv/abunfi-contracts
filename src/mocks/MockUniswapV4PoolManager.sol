...
contract MockUniswapV4PoolManager is IPoolManager {
        ...
        returns (BalanceDelta memory callerDelta, BalanceDelta memory feesAccrued)
    {
        ...
        if (params.liquidityDelta > 0) {
            ...
            callerDelta = BalanceDelta({amount0: int128(uint128(amount0)), amount1: int128(uint128(amount1))});
            ...
            feesAccrued =
                BalanceDelta({amount0: int128(uint128(amount0 / 1000)), amount1: int128(uint128(amount1 / 1000))});
        }
        ...
        returns (BalanceDelta memory)
    {
        ...
        if (params.zeroForOne) {
            delta = BalanceDelta({amount0: int128(uint128(amountIn)), amount1: -int128(uint128(amountOut))});
        } else {
            delta = BalanceDelta({amount0: -int128(uint128(amountOut)), amount1: int128(uint128(amountIn))});
        }
        ...
        returns (BalanceDelta memory)
    {
        ...
        return BalanceDelta({amount0: int128(uint128(amount0)), amount1: int128(uint128(amount1))});
    }
